...
for p in range(1,5+1,1):
    raw = requests.get("https://search.daum.net/search?w=news&nil_"
    "search=btn&DA=NTB&enc=utf8&cluster=y&cluster_page=1&q={s}&p={p}".format(s = search,p = p))
    html = BeautifulSoup(raw.text,'html.parser')

    container = html.select("ul#clusterResultUL li")
    for c in container:

        #기사 제목 : divwrap_tit mg_tit a
        title = c.select_one("div.wrap_tit.mg_tit a").text.strip()
        #기사 요약 : p.f_eb desc
        content = c.select_one("p.f_eb desc")
